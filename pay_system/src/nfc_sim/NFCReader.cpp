...
NFCReader::NFCReader()
    : model_("FreeBuff NFC Reader v1.0"), rng_(std::random_device{}()) {}
...
std::optional<NFCData> NFCReader::readCard() {
    if (!connected_ || !cardPresent_) return std::nullopt;
    NFCData data;
    ...
    for (int i = 0; i < 16; ++i) {
        if (i < 6 || i >= 12) pan << digitDist(rng_);
        else pan << '*';
        if (i % 4 == 3 && i < 15) pan << ' ';
    }
